        ...
        <>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="absolute top-1/2 left-4 right-4 -translate-y-1/2 bg-white rounded-3xl shadow-2xl z-50 overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-gray-900">{t.editFoodStreet}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">{t.streetName}</label><input required value={name} onChange={e => setName(e.target.value)} className="w-full border border-gray-300 rounded-xl p-3" placeholder={t.streetNamePlaceholder} /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">{t.city}</label><input required value={city} onChange={e => setCity(e.target.value)} className="w-full border border-gray-300 rounded-xl p-3" placeholder={t.cityPlaceholder} /></div>
              ...
              <div className="border-t border-gray-100 pt-4">
                <p className="text-sm font-medium text-gray-700 mb-3">{t.mapBoundingBox} <span className="text-xs font-normal text-gray-400">({t.mapBoundingBoxHint})</span></p>
                <div className="grid grid-cols-2 gap-3">
                  {coordInput(t.nwLatitude, latNw, setLatNw, '3.1490')}
                  {coordInput(t.nwLongitude, lonNw, setLonNw, '101.6950')}
                  {coordInput(t.seLatitude, latSe, setLatSe, '3.1460')}
                  {coordInput(t.seLongitude, lonSe, setLonSe, '101.6985')}
                </div>
                {latNw && lonNw && latSe && lonSe && (
                  <p className="text-xs text-blue-600 mt-2">{t.mapImageWillGenerate}</p>
                )}
              </div>

              <div className="flex gap-3 mt-4">
                <button type="button" onClick={onClose} className="flex-1 bg-gray-100 text-gray-800 py-3 rounded-xl font-semibold">{t.cancel}</button>
                <button type="submit" className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-semibold shadow-md shadow-blue-600/20">{t.update}</button>
              </div>
            </form>
          </motion.div>
        </>
