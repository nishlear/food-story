...
import { useLanguage, timeAgo, interpolate, plural } from '../i18n/context';
...
export default function AdminDashboard({ authHeaders }: Props) {
  ...
  const metricCards = [
    {
      label: t.streetFoodCreated,
      value: stats?.streets ?? 0,
      icon: Map,
      badge: t.streetsBadge,
      ...
      <div className="grid grid-cols-3 gap-10">
        ...
        <div className="col-span-2 flex flex-col gap-8">
          ...
          <div className="bg-orange-50 rounded-3xl p-8 flex flex-col gap-6">
            <div className="flex items-end justify-between">
              <div>
                <h2 className="text-xl font-semibold text-gray-900">{t.topPerformingStreets}</h2>
                <p className="text-sm text-gray-500 mt-1">{t.topStreetsSubtitle}</p>
              </div>
              <span className="text-sm font-semibold text-orange-600 cursor-default">{t.viewAllMaps}</span>
            </div>

            <div className="flex flex-col gap-3">
              {streets.length === 0 ? (
                <p className="text-sm text-gray-400 text-center py-4">{t.noStreetsYet}</p>
              ) : (
                streets.map((street, i) => (
                  <div key={street.id} className="bg-white rounded-2xl p-5 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500">{interpolate(plural(street.vendors_count, t.vendorCountOneShort, t.vendorCountManyShort), { count: street.vendors_count })}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-lg font-extrabold text-orange-600">#{i + 1}</p>
                      <p className="text-xs text-gray-400">{t.rankLabel}</p>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="border-2 border-dashed border-gray-200 rounded-3xl h-48 flex items-center justify-center bg-gradient-to-br from-orange-50 to-amber-50">
            <div className="flex flex-col items-center gap-2 text-gray-400">
              ...
              <p className="text-sm italic">{t.vendorActivityChart}</p>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-1 flex flex-col gap-6">
          ...
          <div className="bg-orange-50 rounded-3xl p-8 flex flex-col gap-6 border border-gray-100">
            ...
            {activity.length === 0 ? (
              <p className="text-sm text-gray-400">{t.noActivityYet}</p>
            ) : (
              <div className="relative flex flex-col gap-6 pb-2">
                ...
                {activity.map((comment, i) => (
                  <div key={comment.id} className="pl-10 relative">
                    ...
                    <p className="text-sm font-semibold text-gray-900">{t.newComment}</p>
          ...
          <div className="bg-orange-500 rounded-3xl p-8 relative overflow-hidden shadow-lg shadow-orange-200">
            <div className="relative z-10 flex flex-col gap-2">
              <h3 className="text-lg font-extrabold text-white">{t.needInsights}</h3>
